...
# Get "handle" to the vision sensor
result, vision_sensor_handle = vrep.simxGetObjectHandle(clientID, 'Vision_sensor', vrep.simx_opmode_oneshot_wait)
if result != vrep.simx_return_ok:
...
time.sleep(1)
vrep.simxGetVisionSensorImage(clientID, vision_sensor_handle, 0, vrep.simx_opmode_streaming)
time.sleep(1)
...
Goal_joint_angles = np.array([[-0.5*np.pi,0.8*np.pi,0,0,0,0]])
